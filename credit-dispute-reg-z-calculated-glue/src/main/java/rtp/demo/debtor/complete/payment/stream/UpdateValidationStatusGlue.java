...
public class UpdateValidationStatusGlue {
	...
	public UpdateValidationStatusGlue() {
		...
		KStream<String, String> paymentsStream = builder.stream(targetTopic,
	...
	private void triggerAdhocTask(String key, String value) {
		LOG.info("triggering dispute case create customer notify completion" + key);
		...
		customerProxy.triggerAdhocTask(new Gson().fromJson(val[0], String.class),
				"{\"caseFile_regZCalculatedComplete\":" + "true}");
	}
